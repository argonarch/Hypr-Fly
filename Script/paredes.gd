...
func _on_zona_estado(nombre):
	...
	if zona_salida == "zona-1":
		...
		var command = conectionDataBase("select * from " + tabla + " where item = '" + nombre_sector + "' ;")[0]["command"]
		print("El commando es: " + command)
		if command == "comandado":
			var tabla_nombre = conectionDataBase("select tabla from " + tabla + " where item = '" + nombre_sector + "' ;")[0]["tabla"]
			var icono_nombre = conectionDataBase("select icon from " + tabla + " where item = '" + nombre_sector + "' ;")[0]["icon"]
			self.visible = false
